...
def train(args):
    ...
    if 'validation' in load_dataset(args.dataset_name, args.dataset_config).keys():
    ...
    # 토큰 ID를 하나의 리스트로 합치기
    def concatenate_tokens(tokenized_data, description):
        all_input_ids = []
        for example in tqdm(tokenized_data, desc=description):
            all_input_ids.extend(example['input_ids'])
        return all_input_ids

    all_train_input_ids = concatenate_tokens(tokenized_train, "Concatenating training tokenized texts")
    all_val_input_ids = concatenate_tokens(tokenized_val, "Concatenating validation tokenized texts")
    ...
    # 손실 함수 정의
    loss_fn = nn.CrossEntropyLoss(ignore_index=0)  # 패딩 마스크가 없으므로 필요 시 조정
...
# ====================================================
def main():
    parser = argparse.ArgumentParser(description="Hugging Face 데이터셋을 사용한 GinBBai 모델 학습")
    ...
    parser.add_argument('--tokenizer_name', type=str, default='EleutherAI/gpt-neo-125m', help='사용할 사전 학습된 토크나이저 이름 또는 경로')
